	...
	*/
	...
	glVertexAttribPointer(vNormal, 3, GL_FLOAT, GL_TRUE, 0,
		BUFFER_OFFSET(filevertices.size() * sizeof(float) +
			filevertices.size() / 3 * 4 * sizeof(float)));
	PositionInfo* vw = new PositionInfo(vertexBuffers[1], arrayBuffers[1],
	...
	vw->addScale(2.0f, 2.0f, 2.0f);
	objectList.push_back(vw);
...
void display() {
	...
	vmath::vec3 pointLightPosition(4.0f, 4.0f, 4.0f);
	vmath::vec3 pointLightColor(1.0f, 1.0f, 1.0f);
	vmath::vec3 viewDirection(0.0f, 0.0f, 1.0f);
	vmath::vec3 sum, norm;
	GLuint ambientLocation;

	glUniform3fv(locationMap["eyeDirection"], 1, viewDirection);
	...
	glUniform1f(locationMap["shininess"], 50.0f);
	glUniform1f(locationMap["strength"], 1.0f);
	glUniform3fv(locationMap["pointLightPosition"], 1, pointLightPosition);
	glUniform3fv(locationMap["pointLightColor"], 1, pointLightColor);
	//	projectionTransform = vmath::ortho(-6.0f, 6.0f, -6.0f, 6.0f, -20.0f, 20.0f); 
	// Use a 12x12x12 cube centered at the origin.
	projectionTransform = vmath::perspective(45.0f, 1.0f, 0.1f, 1000.0f);
	...
	viewingTransform = vmath::lookat(vmath::vec3(0.0f, 0.0f, zDistance),
...
int main(int argCount, char* argValues[]) {
	...
	glutStartUp(argCount, argValues, "My Test of New Routines", 600, 600);
